...
public class countSubsetWithSumK {
    ...
    public static int findWays(int arr[], int target) {
        ...
        if (arr[0] == 0)
            dp[0][0] = 2; // arr ke pehla element 0 ho and target bhi 0 ho  to uss array ke first element yani ki 0 ko lekr aur na lekr dono tareeko se answer nikala jaa skata hai issiliye dp[0][0] = 2
        if (arr[0] != 0 && arr[0] <= target)
            dp[0][arr[0]] = 1; // abb is point me array ka pehle element na to 0  hai aur arr[0] ,= target ke to vo khud choose hokar answer de sakta hai to issiliye usey include karna jaruri hai issiliye dp[0][arr[0]] = 1 ye bata raha hai ki agar array ka pehla element hi dekha jae aur target arr[0]  ke barabar ho to usey choose karsakte ho besharte ki vo khud arr[0] se bada ho ya barabar ho
    ...
    public static int findWaysBottomUp(int arr[], int target) {
        ...
        if (arr[0] == 0)
            ...
            dp[0][0] = 1; // arr ka pehle element kuch bhi ho but target 0 ho chuka hai toh matlab usey naa lekr bhi ek tareeka hai answer bnanne ka issiliye dp[0][0] = 1;
        if (arr[0] != 0 && arr[0] <= target)
            dp[0][arr[0]] = 1; // abb is point me array ka pehle element na to 0  hai aur arr[0] ,= target ke to vo khud choose hokar answer de sakta hai to issiliye usey include karna jaruri hai issiliye dp[0][arr[0]] = 1 ye bata raha hai ki agar array ka pehla element hi dekha jae aur target arr[0]  ke barabar ho to usey choose karsakte ho besharte ki vo khud arr[0] se bada ho ya barabar ho
